...
class ModelSkeleton:
  ...
  def __init__(self, mc):
    ...
    self.ph_lidar_input = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 6],
        name='lidar_input'
    )
    ...
    self.q = tf.FIFOQueue(
        ...
        shapes=[[],
                [mc.BATCH_SIZE, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 6],
  ...
  def _context_aggregation_layer(self, layer_name, inputs):
    """
    ...
    with tf.variable_scope(layer_name) as scope:
      ...
      squeeze = self._conv_layer(
          'squeeze', pool, filters=(pool_dim // mc.REDUCTION), size=1, stride=1,
